...
export function getSessionFromReq(token: string | undefined | null): Session {
    if (!token) {
        throw new Error("No authorization header found");
    }
    const verified = jwt.verify(token, livekitApiSecret);
    if (!verified) {
...
function generateroom_name(): string {
